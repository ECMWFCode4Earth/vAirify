...
def dates_without_measurements(
    dates_from_db: list[datetime], potential_dates: list[datetime]
):
...
# (default 14 days). In any given 24-hour period if there are any hour slots that do not
...
def retrieve_dates_requiring_in_situ_data() -> [datetime]:
    ...
    dates = pd.date_range(
        search_start_date, search_end_date, inclusive="right", freq="24h"
    )
