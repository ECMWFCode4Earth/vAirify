...
test.describe("In-situ AQI plots", () => {
  ...
  test("Expect in-situ AQI 3 plot at 00:00 due to multiple stations showing PM2.5", async ({
    ...
    const chartShot = await cityPage.captureChartScreenshot(cityPage.aqiChart);
    await expect(chartShot).toMatchSnapshot("In-situ-AQI-is-3-at-00:00-due-to-PM2.5.png");
  });
  ...
  test("Expect in-situ AQI 2 plot at 00:00 when AQI site 4 is removed", async ({
    ...
    const chartShot = await cityPage.captureChartScreenshot(cityPage.aqiChart);
    await expect(chartShot).toMatchSnapshot("In-situ-AQI-is-2-at-00:00-due-to-PM2.5.png");
  });
  ...
  test("Expect in-situ AQI 1 plot at 00:00 when AQI stations 2,3 and 4 are removed", async ({
    ...
    await cityPage.siteRemover("AQI 4 site");
    const chartShot = await cityPage.captureChartScreenshot(cityPage.aqiChart);
    await expect(chartShot).toMatchSnapshot("In-situ-AQI-is-1-at-00:00-due-to-PM2.5.png");
  });
