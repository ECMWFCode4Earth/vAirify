...
test.describe('Pollutant graphs', () => {
  test.beforeEach(async ({ vairifyCityPage }) => {
    await vairifyCityPage.setupCityPageGraph()
  ...
  test('o3 snapshot', async ({ vairifyCityPage, page }) => {
  ...
  test('o3 snapshot, no Copacabana and Centro', async ({ vairifyCityPage }) => {
    await vairifyCityPage.siteRemover('Copacabana')
    await vairifyCityPage.siteRemover('Centro')
    const chartShot = await vairifyCityPage.captureO3ChartScreenshot()
